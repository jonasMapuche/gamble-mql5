...
//| Class                                                            |
//+------------------------------------------------------------------+
class MorningStar : public CandleStick
  {
    private:
    ...
    public:
      MorningStar(const int value,const string inpattern);
      ~MorningStar();
...
//+------------------------------------------------------------------+
MorningStar::MorningStar(const int value,const string inpattern)
  {
...
//| Write morning star                                               |
//+------------------------------------------------------------------+
void MorningStar::Write(const int value)
  {
...
//| Verify morning star                                              |
//+------------------------------------------------------------------+
bool MorningStar::Main(const int value,const string intimenow,const bool save,const double average)
  {
//---
    int value1=value;
    int value2=value+1;
    ...
    int account_max=5+1;
//---
    double inOpen1,inClose1=0;
    double inOpen2,inClose2=0;
    double inOpen3,inClose3,meanClose3=0;
...
//---
    for(int i=value1; i<=value5; i++) {
      ...
      if(i==value2) if(green(candlestick[i].getOpen(),candlestick[i].getClose())) account[2]=TRUE; else account[3]=TRUE;
...
//---
        if((morningstargreen(inClose3,inClose2,inOpen2,inOpen1)) || (morningstarred(inClose3,inClose2,inOpen2,inOpen1))){
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1001),GetRuleCandle(2223));
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1001),GetRuleCandle(2231));
          thrut[1]=TRUE;
        };
//---
        if(abovehalfbodyred(inOpen3,inClose3,inClose1)){
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1001),GetRuleCandle(2063));
          thrut[2]=TRUE;
        };
//---
