...
//| Class                                                            |
//+------------------------------------------------------------------+
class Hanged : public CandleStick
  {
    private:
    ...
    public:
      Hanged(const int value,const string inpattern);
      ~Hanged();
...
//+------------------------------------------------------------------+
Hanged::Hanged(const int value,const string inpattern)
  {
...
//| Write hanged                                                     |
//+------------------------------------------------------------------+
void Hanged::Write(const int value)
  {
...
//| Verify hanged                                                    |
//+------------------------------------------------------------------+
bool Hanged::Main(const int value,const string intimenow,const bool save,const double average)
  {
//---
...
//---
        if((hammerred(inHigh1,inOpen1,inClose1,inLow1,mean,percent3)) || (hammergreen(inHigh1,inOpen1,inClose1,inLow1,mean,percent3))){
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1004),GetRuleCandle(2002));
          thrut[0]=TRUE;
        };
//---
        if(thrut[0]==TRUE) {
