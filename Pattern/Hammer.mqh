...
//| Class                                                            |
//+------------------------------------------------------------------+
class Hammer : public CandleStick
  {
    private:
      double Mean();
    ...
    public:
      Hammer(const int value,const string inpattern);
      ~Hammer();
...
//+------------------------------------------------------------------+
Hammer::Hammer(const int value,const string inpattern)
  {
...
//| Write hammer                                                     |
//+------------------------------------------------------------------+
void Hammer::Write(const int value)
  {
...
//| Verify hammer                                                    |
//+------------------------------------------------------------------+
bool Hammer::Main(const int value,const string intimenow,const bool save,const double average)
  {
//---
...
//---
        if((hammerred(inHigh1,inOpen1,inClose1,inLow1,mean,percent3)) || (hammergreen(inHigh1,inOpen1,inClose1,inLow1,mean,percent3))){
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1004),GetRuleCandle(2001));
          thrut[0]=TRUE;
        };
//---
        if(thrut[0]==TRUE) {
