...
//| Class                                                            |
//+------------------------------------------------------------------+
class SeatBelt : public CandleStick
  {
    private:
    ...
    public:
      SeatBelt(const int value,const string inpattern);
      ~SeatBelt();
...
//+------------------------------------------------------------------+
SeatBelt::SeatBelt(const int value,const string inpattern)
  {
...
//| Write seat belt                                                  |
//+------------------------------------------------------------------+
void SeatBelt::Write(const int value)
  {
...
//+------------------------------------------------------------------+
//| Verify sealt belt high                                           |
//+------------------------------------------------------------------+
bool SeatBelt::High(const int value,const string intimenow,const bool save,const double average)
  {
//---
...
//---
        if(longgreen(inOpen1,inClose1,mean)){
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1003),GetRuleCandle(2001));
          thrut[0]=TRUE;
        };
//---
        if(sealtbeltgreen(inOpen1,inClose1,inLow1,percent3)){
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1008),GetRuleCandle(2001));
          thrut[1]=TRUE;
        };
//---
...
//+------------------------------------------------------------------+
//| Verify seat belt low                                             |
//+------------------------------------------------------------------+
bool SeatBelt::Low(const int value,const string intimenow,const bool save,const double average)
  {
//---
    double percent3=0.0003;
    int value1=value;
...
//---
        ...
        for(int i=0; i<max; i++) thrut[i]=FALSE;
//---
        if(longred(inOpen1,inClose1,mean)){
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1003),GetRuleCandle(2002));
          thrut[0]=TRUE;
        };
//---
        if(sealtbeltred(inHigh1,inOpen1,inClose1,percent3)){
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1008),GetRuleCandle(2002));
          thrut[1]=TRUE;
        };
//---
        if ((thrut[0]==TRUE) && (thrut[1]==TRUE)) {
