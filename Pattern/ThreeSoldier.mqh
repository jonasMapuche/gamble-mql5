...
//| Class                                                            |
//+------------------------------------------------------------------+
class ThreeSoldier : public CandleStick
  {
    private:
    ...
    public:
      ThreeSoldier(const int value,const string inpattern);
      ~ThreeSoldier();
...
//+------------------------------------------------------------------+
ThreeSoldier::ThreeSoldier(const int value,const string inpattern)
  {
...
//| Write three soldier                                              |
//+------------------------------------------------------------------+
void ThreeSoldier::Write(const int value)
  {
...
//+------------------------------------------------------------------+
//| Verify three soldier high                                        |
//+------------------------------------------------------------------+
bool ThreeSoldier::High(const int value,const string intimenow,const bool save,const double average)
  {
//---
...
//---
    for(int i=value1; i<=value3; i++) {
      ...
      if((account[3]==TRUE) && (account[2]==TRUE) && (account[1]==TRUE)) {
//---   
...
//---
        if((longgreen(inOpen2,inClose2,mean)) && (inClose2>inClose3)){
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1003),GetRuleCandle(2093));
          thrut[1]=TRUE;
        };
//---
        if((longgreen(inOpen1,inClose1,mean)) && (inClose1>inClose2)){
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1003),GetRuleCandle(2092));
          thrut[2]=TRUE;
        };
//---
...
//+------------------------------------------------------------------+
//| Verify three soldier low                                         |
//+------------------------------------------------------------------+
bool ThreeSoldier::Low(const int value,const string intimenow,const bool save,const double average)
  {
//---
    int value1=value;
    int value2=value+1;
...
//---
        if(longred(inOpen2,inClose2,mean) && (inClose2<inClose3)){
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1003),GetRuleCandle(2103));
          thrut[1]=TRUE;
        }
//---
        if(longred(inOpen1,inClose1,mean) && (inClose1<inClose2)){
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1003),GetRuleCandle(2102));
          thrut[2]=TRUE;
        }
//---
        if ((thrut[0]==TRUE) && (thrut[1]==TRUE) && (thrut[2]==TRUE)) {
