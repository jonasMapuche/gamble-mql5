...
//| Class                                                            |
//+------------------------------------------------------------------+
class SouthernStar : public CandleStick
  {
    private:
    ...
    public:
      SouthernStar(const int value,const string inpattern);
      ~SouthernStar();
...
//+------------------------------------------------------------------+
SouthernStar::SouthernStar(const int value,const string inpattern)
  {
...
//+------------------------------------------------------------------+
//| Write southern star low                                          |
//+------------------------------------------------------------------+
void SouthernStar::Write(const int value)
  {
...
//+------------------------------------------------------------------+
//| Verify southern star                                             |
//+------------------------------------------------------------------+
bool SouthernStar::Main(const int value,const string intimenow,const bool save,const double average)
  {
//---
...
//---
        if(longlegred(inHigh3,inOpen3,inClose3,inLow3,mean,percent3)){
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1009),GetRuleCandle(2001));
          thrut[0]=TRUE;
        };
//---
        if(inOpen2<inOpen3){
...
//---
        if((marubozured(inHigh1,inOpen1,inClose1,inLow1,percent9)) && (insidebodyred(inOpen2,inClose2,inHigh1,inLow1))){
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1006),GetRuleCandle(2202));
          thrut[3]=TRUE;
        };
//---
