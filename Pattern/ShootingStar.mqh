...
//| Class                                                            |
//+------------------------------------------------------------------+
class ShootingStar : public CandleStick
  {
    private:
      double Mean();
    ...
    public:
      ShootingStar(const int value,const string inpattern);
      ~ShootingStar();
...
//+------------------------------------------------------------------+
ShootingStar::ShootingStar(const int value,const string inpattern)
  {
...
//| Write shooting star                                              |
//+------------------------------------------------------------------+
void ShootingStar::Write(const int value)
  {
...
//+------------------------------------------------------------------+
//| Verify shooting star                                             |
//+------------------------------------------------------------------+
bool ShootingStar::Main(const int value,const string intimenow,const bool save,const double average)
  {
//---
...
//---
        if((hammerinvertedred(inHigh1,inOpen1,inClose1,inLow1,mean,percent3)) || (hammerinvertedgreen(inHigh1,inOpen1,inClose1,inLow1,mean,percent3))){
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1005),GetRuleCandle(2002));
          thrut[0]=TRUE;
        };
//---
        if (thrut[0]==TRUE) {
