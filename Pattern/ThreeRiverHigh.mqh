...
//| Class                                                            |
//+------------------------------------------------------------------+
class ThreeRiverHigh : public CandleStick
  {
    private:
    ...
    public:
      ThreeRiverHigh(const int value,const string inpattern);
      ~ThreeRiverHigh();
...
//+------------------------------------------------------------------+
ThreeRiverHigh::ThreeRiverHigh(const int value,const string inpattern)
  {
...
//+------------------------------------------------------------------+
//| Write triple river high                                          |
//+------------------------------------------------------------------+
void ThreeRiverHigh::Write(const int value)
  {
...
//+------------------------------------------------------------------+
//| Verify triple river high                                         |
//+------------------------------------------------------------------+
bool ThreeRiverHigh::Main(const int value,const string intimenow,const bool save,const double average)
  {
//---
    ...
    int account_max=6+1;
//---
    double inClose1=0,inLow1=0;
    double inOpen2=0,inClose2=0,inHigh2,inLow2=0;
...
//---
    for(int i=value1; i<=value6; i++) {
      ...
      if(i==value2) if(red(candlestick[i].getOpen(),candlestick[i].getClose())) account[2]=TRUE; else account[2]=TRUE;
...
//---
        if((hammerred(inHigh2,inOpen2,inClose2,inLow2,mean,percent3)) || (hammergreen(inHigh2,inOpen2,inClose2,inLow2,mean,percent3))){
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1004),GetRuleCandle(2000));
          thrut[1]=TRUE;
        };
//---
        if(((inClose1<inClose2) && red(inOpen2,inClose2)) || ((inClose1<inOpen2) && green(inOpen2,inClose2))) {
          if(save) sqlite.SaveRule(intimenow,inPattern,inSymbol,inTime,GetTypeCandle(1004),GetRuleCandle(2102));
          thrut[2]=TRUE;
        };
//---
        if(inLow1>inLow2){
