SELECT MES_1.PATTERN,
  ...
  FROM (
           SELECT ViewPattern.PATTERN,
                  CASE WHEN BUY_MONTH.QUANTITY IS NULL THEN 0 ELSE BUY_MONTH.QUANTITY END AS QUANTITY
             FROM (
                      SELECT PATTERN,
                             MONTH,
                             COUNT(1) AS QUANTITY
                                 ...
                                 SELECT PATTERN,
                                   ...
                                   FROM (
                                            SELECT CAST (strftime('%m', DATE_EVENT) AS DECIMAL) AS MONTH,
                                              ...
                                              FROM (
                                                       SELECT PATTERN,
                                                              DATE(substr(date, 1, 4) || '-' || substr(date, 6, 2) || '-' || substr(date, 9, 2) ) AS DATE_EVENT
                                                         FROM ViewOrderSellStopTrue
                                                   )
                                                   ORDER_BUY
                                        )
                                        BUY_TRUE
                  ...
                  BUY_MONTH
                  RIGHT OUTER JOIN
                  ViewPattern ON ViewPattern.PATTERN = BUY_MONTH.PATTERN
